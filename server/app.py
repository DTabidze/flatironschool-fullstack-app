...
def enrollments():
    ...
    if "student_id" not in data or "course_id" not in data:
        return {"error":"You must include include student id and course id"},422
    with app.app_context():
        if "sqlite" in app.config['SQLALCHEMY_DATABASE_URI']:
            event.listen(db.engine,"connect",fk_connect)
...
@app.route("/enrollments/<int:student_id>:<int:course_id>",methods=["DELETE"])
def enrollment_by_id(student_id,course_id):
